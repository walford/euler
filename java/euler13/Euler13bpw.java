...
// one-hundred 50-digit numbers (as seen in nums matrix).

import static java.lang.System.out;

public class Euler13bpw {

	public static void main(String[] args) {

		long begin = System.currentTimeMillis(); //begin time measurement 
        ...
        int[] answer_digit_list = new int[nums[0].length];
        ...
        for(int i=nums[0].length-1; i>=0; i--) {
            ...
            if(i > 0) {
                int[] add_break = add_breaker(currentAdd);
                answer_digit_list[i] += add_break[1];
                answer_digit_list[i-1] += add_break[0];
                if(answer_digit_list[i] > 9) {
                    int[] add_push = add_breaker(answer_digit_list[i]);
                    answer_digit_list[i] = add_push[1];
                    answer_digit_list[i-1] += add_push[0];
                }   
            } else {
                answer_digit_list[i] += currentAdd;
            }

        }

        for(int i : answer_digit_list) {
            out.print(i);
        }
        out.println();
    ...
    static int[] add_breaker(int num) {
